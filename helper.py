...
def data_over_time(df, col):
    nation_over_time = df.drop_duplicates(['Year', col])['Year'].value_counts().sort_values().reset_index()
...
def country_event_heatmap(df,country):
    ...
    pt = new_temp_df.pivot_table(index='Sport', columns='Year', values='Medal', aggfunc='count').fillna(0)
    return pt
...
def most_successful_countrywise(df, country):
    ...
    x = temp_df['Name'].value_counts().reset_index().merge(df, left_on='index', right_on='Name', how='left')[
        ['index', 'Name_x', 'Sport']].drop_duplicates('index').head(15)
    x.rename(columns={'index': 'Name', 'Name_x': 'Medals'}, inplace=True)
    return x
